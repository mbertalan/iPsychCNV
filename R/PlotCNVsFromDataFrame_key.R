...
##'
##' @title PlotCNVsFromDataFrame_key
...
##' @param key - exchange the ID printed on the plot with a deidentified ID - requires that the DF contains a column called ID_deidentified 
##' @author Marcelo Bertalan
##' @export

PlotCNVsFromDataFrame_key <- function(DF, PathRawData=".", Cores=1, Skip=0, PlotPosition=1, Pattern="*",recursive=TRUE, dpi=300, Files=NA, ByFolder=FALSE, SNPList=NULL, key=NA) # Path from cluster  
{
  library(ggplot2)
  ...
  mclapply(DF$UniqueID, mc.cores=Cores, function(UID) 
  {
    ...
    sample <- ReadSample_snplist(RawFile, skip=Skip, SNPList=SNPList)
