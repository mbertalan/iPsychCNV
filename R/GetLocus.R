...
##' @param Url: If you can access https, default FASLE, only for hg19.
...
GetLocus <- function(df, hg="hg19", Url=FALSE) # df with CHR=chr12, CNV_Start=numeric, CNV_Stop=numeric
{
	library(biovizBase)
	library(BiocGenerics)
	data(hg19IdeogramCyto)
	
	# Computerome do not allow closes the connection if htpps.
	if(Url)
	{
		IdeogramCyto <- getIdeogram(hg, cytoband = TRUE)
		tmp <- BiocGenerics::as.data.frame(IdeogramCyto)
	}
	else
	{
