...
iPsychCNV <- function(PathRawData = "/media/NeoScreen/NeSc_home/ILMN/iPSYCH/", MINNumSNPs=100, Cores=10, NumFiles="All", Pattern="22q11_*", MinLength=100000, SelectedFiles=NA, Skip=10, LCR=TRUE, PFB=NULL, chr=NA, penalty=60, Quantile=TRUE, QSpline=TRUE, sd=0.18, recursive=FALSE, CPTmethod="meanvar", CNVSignal=0.1) # Files2 OutputPath
{	
	...
	tmp <- mclapply(Files[1:NumFiles], mc.cores=Cores, mc.preschedule = FALSE, function(X) 
	{
		...
		### FIND CNVs ###
		ptm.tmp <- proc.time()
		CNVs <- FindCNV.V4(ID, MINNumSNPs, Sample, CPTmethod=CPTmethod, CNVSignal=CNVSignal)
		Res.tmp <- proc.time() - ptm.tmp
