...
GetDataVariables <- function(Data)
{
	...
	res2 <- data.frame(CNVmean=CNVmean, CNVmeanOrig=CNVmeanOrig, CNVmeanByRef=CNVmeanByRef, UpMean=HighMean, DownMean=LowMean, SDCNV=SDCNV) # SDMeansCNV=SDMeansCNV, SDHigh=SDHigh, SDLow=SDLow,VarianceCNV=VarianceCNV, DiffHigh=DiffHigh, DiffLow=DiffLow, CNVmeanOrig=CNVmeanOrig
	return(res2)	
